...
class TestEsgvocHomePaths:
    ...
    def test_user_dir_created(self, home):
        # user_dir is now an alias for root
        assert home.user_dir == home.root

    def test_user_dbs_dir_created(self, home):
        # user_dbs_dir is now an alias for dbs_dir
        assert home.user_dbs_dir.exists()
        assert home.user_dbs_dir == home.root / "dbs"

    def test_user_state_file_path(self, home):
        # user_state_file is a legacy path; no real state.json is written any more
        assert home.user_state_file == home.root / "dbs" / "_state_legacy.json"
        assert not home.user_state_file.exists()

    def test_user_cache_dir_created(self, home):
        # user_cache_dir delegates to registry_cache_dir (XDG cache)
