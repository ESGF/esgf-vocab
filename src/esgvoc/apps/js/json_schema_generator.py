...
def _process_composite(collection: Collection) -> str:
    result = ""
    for term in collection.terms:
        if term.kind == TermKind.PATTERN:
            result += term.specs[PATTERN_JSON_KEY]
        else:
            raise NotImplementedError(f'{term.kind} term is not supported yet')
    return result


def _match_collection(field: str, collections: list[Collection]) -> tuple[str | None, str | list | None]:
...
def _generate_property(project_id: str, collections: list[Collection], schema_field: str) -> tuple[str, dict]:
    ...
    if LONG_NAME_POSTFIX in schema_field:
        ...
        property_key, property_value = _match_collection(recomputed_schema_field, collections)
    else:
        # 2. Process schema_fields that are collection ids. (e.g. of sub_experiment_id).
        property_key, property_value = _match_collection(schema_field, collections)
...
def generate_json_schema(project_id: str) -> str:
    ...
    if template_file_path.exists():
        ...
        connection = projects._get_project_connection(project_id)
        properties = list()
