...
# ---------------------------------------------------------------------------
# install  (admin install — copy local build into the DB store)
# ---------------------------------------------------------------------------

_REGISTRY_NAME_RE = re.compile(r"^(v\d+\.\d+\.\d+.*|latest|dev-latest)$")


@app.command(name="install")
def admin_install(
    project_id: str = typer.Argument(..., help="Project ID (e.g. 'cmip7')."),
    db_path: Path = typer.Argument(..., help="Path to the locally built .db file."),
    name: str = typer.Option(
        "local", "--name", "-n",
        help=(
            "Name to store the DB under (e.g. 'my-experiment', 'fix-42'). "
            "Defaults to 'local'. "
            "Names matching vX.X.X, 'latest', 'dev-latest' are reserved for the registry."
        ),
    ),
    activate: bool = typer.Option(
        False, "--activate",
        help="Also activate this version after installing (runs esgvoc use).",
    ),
):
    """
    Install a locally built database into the esgvoc DB store.

    \b
    Install under default name 'local':
      esgvoc admin install cmip7 ./cmip7.db
    ...
    Install and activate immediately:
      esgvoc admin install cmip7 ./cmip7.db --name feature-branch --activate
    ...
    if _REGISTRY_NAME_RE.match(name):
        console.print(
            f"[red]Reserved name:[/red] '{name}' matches the registry naming pattern.\n"
            "Choose a non-version name (e.g. 'local', 'my-experiment', 'fix-42')."
        )
        raise typer.Exit(1)
    ...
    if activate:
        ...
        console.print(f"[dim]Run 'esgvoc use {project_id}@{name}' to activate.[/dim]")
